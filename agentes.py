...
class NICRetriever:
    ...
    basada en rutas (Kaggle), y la inicialización directa por objetos (Streamlit).
    Evita conflictos de parámetros de firmas (TypeError).
    """
    def __init__(self, index=None, metadata=None, embedding_model=None, index_path=None, metadata_path=None, model_path=None):
        ...
        # Escenario B: Inicialización clásica por rutas de archivos (Kaggle Experimentation)
        elif index_path is not None and metadata_path is not None and model_path is not None:
            import faiss
            from sentence_transformers import SentenceTransformer
            
            print("📂 [Retriever] Cargando motor nativo FAISS por rutas...")
    ...
    def buscar(self, query_rag: str, k: int = 5) -> List[Dict]:
        ...
        # Ejecución en Modo Streamlit / LangFace Vectorstore wrapper
        if getattr(self, "es_modo_langchain", True):
            # El objeto 'embedding_model' aquí es en realidad el vectorstore de FAISS
        ...
        # Ejecución en Modo Kaggle / Native FAISS index execution
        else:
